...
var worm_parameters = {
    ...
    min_r: 40, 
    max_r: 40,
    init_left: Math.random()*$(window).width(), 
    init_top: 0, 
    init_rotation: 90,
    all_pipes: false,
    path_mode: false,
...
//DOM builder
function build_DOM(seq, init_top, init_left, init_rotation, thickness, color) {

    var full_500s = Math.floor(seq.length/500);
    var remainder_pipes = seq.length%500;
    ...
    for (var j=0; j<full_500s; j++) {
        ...
        for (var i=499; i>=0; i--) {
            var seq_i = seq[i+(j*500)];
            var prev_pipe = seq[i+(j*500)-1];           
        ...
        if (j == 0) {
            ...
            var last_wrap = $(".worm"+worm_counter).find(".wrap")[j*500 - 2]
            $(last_wrap).append(tree);
        }

    } /////end of full_500s loop ///////

    //remainder loop/////
    var tree = "";

    for (var i=remainder_pipes-1; i>=0; i--) {

        var seq_i = seq[i+(full_500s*500)];
        var prev_pipe = seq[i+(full_500s*500)-1];           
        if (seq_i.type == "straight") {
            var new_pipe = '<div class="straight"></div>';   
        }
        else {
    ...
    if (full_500s == 0) {
        ...
        var last_wrap = $(".worm"+worm_counter).find(".wrap")[full_500s*500 - 2]
        $(last_wrap).append(tree);
    }
...
function build_seq_DOM_and_animate(p) {
    var seq = build_seq(p.pipe_number, p.min_s, p.max_s, p.min_t, p.max_t, p.thickness, p.speed, p.all_pipes, p.min_r, p.max_r, p.init_top, p.init_left, p.init_rotation, p.bounds, p.path_mode);  
    build_DOM(seq, p.init_top, p.init_left, p.init_rotation, p.thickness, p.color);    
    animate_worm(seq);   
    worm_counter++;
}
...
//begin a worm
$("#squig_but").on("click", function() {
    worm_parameters.init_left = Math.random()*$(window).width();
    worm_parameters.init_top = 0;
    worm_parameters.init_rotation = 90;
    worm_parameters.color = "#F3A748";
    build_seq_DOM_and_animate(worm_parameters);  

});

$("#sqwag_but").on("click", function() {
    worm_parameters.init_left = Math.random()*$(window).width();
    worm_parameters.init_top = $(window).height();
    worm_parameters.init_rotation = -90;
    worm_parameters.color = "#72C0AA";
    build_seq_DOM_and_animate(worm_parameters);  
});
